    ...
    >
      <div className="container mx-auto px-4 lg:px-6">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          {/* Desktop Navigation */}
          <div className="hidden lg:flex items-center gap-0.5 xl:gap-1 absolute left-[47%] xl:left-[49%] -translate-x-1/2 bg-background/50 backdrop-blur-sm px-1.5 py-1.5 rounded-full border border-border/40 shadow-sm w-max">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={cn(
                  "relative px-3 xl:px-4 py-2 rounded-full text-xs xl:text-sm font-medium transition-all duration-300",
